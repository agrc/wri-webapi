...
namespace wri_webapi
{
    ...
    {
        public HistoricalModule(IQuery queries) : base("/historical")
        {
            Put["/project/{id:int}/create-related-data", true] = async (_, ctx) =>
            {
                ...
                using (var connection = new SqlConnection(connectionString))
                {
                    ...
                    const string geometryQuery = "SELECT [Shape] shape, [FeatureId], [TypeDescription] category FROM {0} WHERE [Project_ID] = @id";

                    using (var transaction = new TransactionScope(TransactionScopeOption.RequiresNew, TimeSpan.FromSeconds(120), TransactionScopeAsyncFlowOption.Enabled))
                    {
                        foreach (var table in new[] {"POINT", "LINE", "POLY"})
                        {
                            ...
                            foreach (var feature in features)
                            {
                                ...
                                await connection.ExecuteAsync("delete from [wri].[dbo].county WHERE [featureId] = @id;" +
                                                        ...
                                                        "delete from [wri].[dbo].LANDOWNER WHERE [featureId] = @id", new
                                                        {
                                                            id = primaryKey
                                                        });
